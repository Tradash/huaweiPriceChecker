...
// process.setMaxListeners(urls.length > 10 ? urls.length + 10 : 10);
...
const priceCrawler = async () => {
  ...
  for (let i = 0; i < urls1.length; i++) {
    rez[urls1[i].name] = {};
    // if (i===0) {
    //   rez = {model:urls1[i].name}
    // } else {
    //   rez[]
    // }
    for (let j = 0; j < urls1[i].shops.length; j++) {
      // console.log("Стартую воркер", i, j);
      activeWorker++;
      ...
      newWorker.on("message", (data) => {
        ...
        // if (rez[urls1[i].name].hasOwnProperty(urls1[i].shop[j])) {
        //   rez[urls1[i].shop[j]] = data === null ? "Нет в наличии" : data;
        // }
        // console.log("Убиваю воркер", i, j, activeWorker);
        newWorker.terminate();
      });
    }
  }
